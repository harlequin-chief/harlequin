...
const MAX_NUMBER_OF_SAME_REQUESTS_PER_PEER: usize = 8;
...
mod rep {
	...
	///
	/// HARLEQUIN PATCH (, ticket 3154fe0d hunk 5): was `new_fatal` = instant ban. A peer
	/// re-joining after a link flap legitimately re-sends the request it lost mid-flight, and the
	/// fatal ban read that retry as malice (the bootnode banned the tablet this way at 11:35 UTC).
	/// Heavy-but-decaying penalty instead: real spammers still accumulate to disconnection through
	/// the reputation system; a single post-flap retry no longer executes the peer.
	...
	///
	/// HARLEQUIN PATCH (, hunk 3): was `-(1 << 10)`, which stacked to a disconnection in
	/// four or five repeats — twelve-second retries meant a peer we could not answer was gone inside
	/// a minute. This is the twin of the 21-jul patch above: on that day we softened the FATAL ban of
	/// `SAME_REQUEST` and left this one untouched, so the same class of bug bit again from the other
	/// side. A quarter of the old weight still adds up for a genuine spammer and no longer executes a
	/// peer for asking twice too often.
	pub const SAME_SMALL_REQUEST: Rep =
		Rep::new(-(1 << 8), "same small block request multiple times");
}
	...
	Client: HeaderBackend<B> + BlockBackend<B> + Send + Sync + 'static,
{
	...
	fn handle_request(
		...
		let maybe_block_response = if reputation_change.is_none() || small_request {
			...
			let asked_for_justification = attributes.contains(BlockAttributes::JUSTIFICATION);
			let served_a_justification = block_response.blocks.iter().any(|b| {
				!b.justification.is_empty() || !b.justifications.is_empty() || b.is_empty_justification
			});
			let justification_shortfall = asked_for_justification && !served_a_justification;

			// If any of the blocks contains any data, we can consider it as successful request.
			if !justification_shortfall
				&& block_response
					.blocks
					.iter()
