...
| 35 | **First constitutional upgrade executed on the live chain through the Key** (2026-07-24): proposed block 26798 (1 founding seat), quorum 3-of-5 co-signed (custody seat cannot sign), 14,400-block objection window served, applied block 41367 — permissionless + fee-exempt by design once the window elapses (submitted from a newborn zero-balance account; `blake2_256(code)` must equal the announced hash). Runtime `spec_version 1→2`; first epoch rotation under the new runtime (block 43200) crossed with finality uninterrupted. Operational lesson recorded: a node whose local finalized state predates the applied upgrade must sync past the apply block on its previous binary before swapping — the startup coherence check reads the local database | live mainnet, two independently operated watchers | — | ✅ runtime `code_hash e95b2648…`, upgraded node binary sha256 `3ac9f1f03d6e6ea96958514dfc90a2a859f98adb7321b96d76a843559d0faf4d` |
